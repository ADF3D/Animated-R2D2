...
Objet3DStatique::Objet3DStatique(std::string nomFichier)
{
    ...
    bool creationListeAffichage = false;
        ...
        while(getline(fichier, ligne))
        {
            ...
            else if (0 == ligne.compare(0, 1, "{"))
            {
                ...
                while(getline(fichier, ligne))
                {
                    ...
                    {
                        // ATTENTION : N'ayant pas enregistré la taille de la texture, j'ai entré la taille 256 en dur
                        // Cette taille n'est valable que pour les faces de la skybox
                        glTexCoord2f((float)x2D / 256, (256 - (float)y2D) / 256); glVertex3d(x3D, y3D, z3D);
                    }
